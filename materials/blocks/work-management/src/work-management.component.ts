  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  currentLabelSize: labelSize = 'large';
  ...
  ngAfterViewInit(): void {
    this.resizeSub = fromEvent(window, 'resize')
      ...
      .subscribe((si) => {
        if (
          (si.target as any).innerWidth <= 580 &&
          (si.target as any).innerWidth > 435
        ) {
          this.currentLabelSize = 'normal';
        } else if ((si.target as any).innerWidth <= 435) {
          this.currentLabelSize = 'small';
        } else {
          this.currentLabelSize = 'large';
        }
        this.quadrantWidth = this.workItemElement.clientWidth * 0.8;
      });
